<template>
  <div class="config-container">
    <el-tabs type="border-card">
      <el-form :inline="true" :model="searchParam" class="demo-form-inline head-form">
          <el-form-item label="配置标题">
            <el-input clearable v-model="searchParam.title" size="small" placeholder="请输入配置标题"
                      style="width: 200px"/>
          </el-form-item>
          <el-form-item>
            <el-button :loading="tableLoading" @click="search" size="small" icon="el-icon-search" type="primary">搜索
            </el-button>
            <el-button @click="configInfo()" size="small" icon="el-icon-plus" type="primary">添加配置
            </el-button>
        ...
        <el-table stripe ref="table" :height="tableHeight" v-loading="tableLoading"
                  :data="configList" border row-key="id">
          ...
          <el-table-column show-overflow-tooltip prop="tag" label="配置标签" align="center"/>
          <el-table-column show-overflow-tooltip prop="groupName" label="配置分组" align="center"/>
          <el-table-column show-overflow-tooltip prop="typeName" label="配置类型" align="center"/>
            ...
            <template slot-scope="scoped">
              <el-row>
                <el-tooltip v-if="isAdmin||btnFunc.some(btn=>btn=='edit')" class="item" effect="dark" content="编辑" placement="bottom">
                  <el-button type="primary" size="mini" icon="el-icon-edit" @click="configInfo(scoped.row)"/>
                </el-tooltip>
                <el-tooltip v-if="!isAdmin||btnFunc.some(btn=>btn=='drop')" class="item" effect="dark" content="删除" placement="bottom">
                  <el-button :loading="delBtnLoading" type="danger" size="mini" class="el-icon-delete" @click="configDel(scoped.row)"/>
                </el-tooltip>
              </el-row>
            </template>
          </el-table-column>
    ...
    <!-- 编辑表单 -->
    <el-dialog :close-on-press-escape="false" :close-on-click-modal="false" center :visible.sync="addDialogVisible" :title="dialogTitle+'配置'">
      <editForm ref="editForm" :infoForm="infoForm" :typeList="typeList" :groupList="groupList" style="width: 100%" @submit="submit" @closeWin="closeWin"></editForm>
    </el-dialog>
  ...
  export default {
    ...
    methods: {
      ...
      getConfigList() {
        ...
        configApi.configList(searchParam).then(res => {
      ...
      configInfo(data){
        ...
        this.$nextTick(function () {
          try {
            configApi.configInfo({id: data.id}).then(res => {
      ...
      configDel(data) {
        this.$confirm('您确定要删除配置分组吗？', '提示', {
          ...
          configApi.configDel({id: data.id}).then(res => {
            this.$message({
              type: 'success',
              message: '删除成功！'
            })
      ...
      // 获取基础数据
      getParamList() {
        configApi.getParamList().then(res => {
      ...
      // 获取配置分组
      getGroupList() {
        configApi.getGroupList().then(res => {
      ...
      setStatus(data) {
        ...
        this.$confirm(content, '提示', {
          ...
          this.tableLoading = true
          configApi.configStatus(params).then(res => {
            this.$message({
              type: 'success',
              message: '操作成功！'
            })
            ...
            for (let i = 0; i < configList.length; i++) {
              if (configList[i].id == data.id) {
                configList[i].status = data.status == 1 ? 2 : 1
              }
            }
            this.configList = Object.assign([], configList)
            this.tableLoading = false
          })
        }).catch(() => {
          let configList = JSON.parse(JSON.stringify(this.configList))
