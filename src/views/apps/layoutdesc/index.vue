<template>
  <div class="layoutDesc-container">
    <el-tabs type="border-card">
      <el-form :inline="true" :model="searchParam" class="demo-form-inline head-form">
          <el-form-item label="推荐描述名称">
            <el-input clearable v-model="searchParam.name" size="small" placeholder="请输入推荐描述名称"
                      style="width: 200px"/>
          </el-form-item>
          <el-form-item>
            <el-button :loading="tableLoading" @click="search" size="small" icon="el-icon-search" type="primary">搜索
            </el-button>
            <el-button @click="layoutDescInfo()" size="small" icon="el-icon-plus" type="primary">添加推荐描述
            </el-button>
        ...
        <el-table stripe ref="table" :height="tableHeight" v-loading="tableLoading"
                  :data="layoutDescList" border row-key="id">
          ...
          <el-table-column show-overflow-tooltip prop="name" label="描述名称" align="center"/>
          <el-table-column show-overflow-tooltip prop="locDesc" label="位置描述" align="center"/>
          <el-table-column show-overflow-tooltip prop="locId" label="位置编号" align="center"/>
            ...
            <template slot-scope="scoped">
              <el-row>
                <el-tooltip v-if="isAdmin||btnFunc.some(btn=>btn=='edit')" class="item" effect="dark" content="编辑" placement="bottom">
                  <el-button type="primary" size="mini" icon="el-icon-edit" @click="layoutDescInfo(scoped.row)"/>
                </el-tooltip>
                <el-tooltip v-if="!isAdmin||btnFunc.some(btn=>btn=='drop')" class="item" effect="dark" content="删除" placement="bottom">
                  <el-button :loading="delBtnLoading" type="danger" size="mini" class="el-icon-delete" @click="layoutDescDel(scoped.row)"/>
                </el-tooltip>
              </el-row>
            </template>
          </el-table-column>
    ...
    <!-- 编辑表单 -->
    <el-dialog :close-on-press-escape="false" :close-on-click-modal="false" center :visible.sync="addDialogVisible" :title="dialogTitle+'推荐描述'">
      <editForm ref="editForm" :infoForm="infoForm" :itemList="itemList" style="width: 100%" @submit="submit" @closeWin="closeWin"></editForm>
    </el-dialog>
  ...
  export default {
    ...
    methods: {
      ...
      getLayoutDescList() {
        ...
        layoutDescApi.layoutDescList(searchParam).then(res => {
          this.layoutDescList = res.data.records
      ...
      layoutDescInfo(data){
        ...
        this.$nextTick(function () {
          try {
            layoutDescApi.layoutDescInfo({id: data.id}).then(res => {
      ...
      // 删除记录
      layoutDescDel(data) {
        this.$confirm('您确定要删除推荐描述吗？', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          this.delBtnLoading = true
          layoutDescApi.layoutDescDel({id: data.id}).then(res => {
            this.$message({
              type: 'success',
              message: '删除成功！'
            })
      ...
      // 获取职级
      getItemList() {
        layoutDescApi.getItemList().then(res => {
      ...
      setStatus(data) {
        ...
        this.$confirm(content, '提示', {
          ...
          this.tableLoading = true
          layoutDescApi.layoutDescStatus(params).then(res => {
            this.$message({
              type: 'success',
              message: '操作成功！'
            })
            ...
            let layoutDescList = JSON.parse(JSON.stringify(this.layoutDescList))
            for (let i = 0; i < layoutDescList.length; i++) {
              if (layoutDescList[i].id == data.id) {
                layoutDescList[i].status = data.status == 1 ? 2 : 1
              }
            }
            this.layoutDescList = Object.assign([], layoutDescList)
            this.tableLoading = false
          })
        }).catch(() => {
          let layoutDescList = JSON.parse(JSON.stringify(this.layoutDescList))
