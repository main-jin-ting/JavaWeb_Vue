<template>
  <div class="info-form">
    <el-form ref="infoForm" :model="infoForm" size="small" label-width="100px">
      <el-form-item label="职级名称" prop="name" :rules="[{
                    required: true, message: '职级名称名称不能为空', trigger: 'blur'
                  },{min: 2, max: 20, message: '职级名称长度在2 到 20 个字符', trigger: 'blur'}]">
        <el-input maxlength="20" clearable v-model="infoForm.name" size="small" placeholder="请输入职级名称"/>
      </el-form-item>
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            levelApi[method](this.infoForm).then(res => {
