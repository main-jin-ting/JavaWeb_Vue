    ...
    <el-tabs type="border-card">
      <el-form :inline="true" :model="searchParam" class="demo-form-inline head-form">
          <el-form-item label="职级名称">
            <el-input clearable v-model="searchParam.name" size="small" placeholder="请输入职级名称"
                      style="width: 200px"/>
          </el-form-item>
          <el-form-item>
            <el-button :loading="tableLoading" @click="handleQuery" size="small" icon="el-icon-search" type="primary">搜索
            </el-button>
            <el-button @click="levelInfo()" size="small" icon="el-icon-plus" type="primary">添加职级
            </el-button>
            ...
            <el-button @click="initSearchParam();handleQuery()" icon="el-icon-refresh-right"
        ...
        <el-table stripe ref="table" :height="tableHeight" v-loading="tableLoading"
                  :data="levelList" border row-key="id">
            ...
            <template slot-scope="scoped">
              <el-row>
                <el-tooltip v-if="isAdmin||btnFunc.some(btn=>btn=='edit')" class="item" effect="dark" content="编辑" placement="bottom">
                  <el-button type="primary" size="mini" icon="el-icon-edit" @click="levelInfo(scoped.row)"/>
                </el-tooltip>
                <el-tooltip v-if="!isAdmin||btnFunc.some(btn=>btn=='drop')" class="item" effect="dark" content="删除" placement="bottom">
                  <el-button :loading="delBtnLoading" type="danger" size="mini" class="el-icon-delete" @click="levelDel(scoped.row)"/>
                </el-tooltip>
              </el-row>
            </template>
          </el-table-column>
...
<script>
  import levelApi from '@/api/permission/level'
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        levelApi.levelList(searchParam).then(res => {
      ...
      levelInfo(data){
        ...
        this.$nextTick(function () {
          try {
            levelApi.levelInfo({id: data.id}).then(res => {
              debugger
      ...
      levelDel(data) {
        this.$confirm('您确定要删除职级吗？', '提示', {
          ...
          levelApi.levelDel({id: data.id}).then(res => {
            this.$message({
              type: 'success',
              message: '删除成功！'
            })
      ...
      setStatus(data) {
        ...
        this.$confirm(content, '提示', {
          ...
          this.tableLoading = true
          levelApi.levelStatus(params).then(res => {
            this.$message({
              type: 'success',
              message: '操作成功！'
            })
            ...
            for (let i = 0; i < levelList.length; i++) {
              if (levelList[i].id == data.id) {
                levelList[i].status = data.status == 1 ? 2 : 1
              }
            }
            this.levelList = Object.assign([], levelList)
            this.tableLoading = false
          })
        }).catch(() => {
          let levelList = JSON.parse(JSON.stringify(this.levelList))
