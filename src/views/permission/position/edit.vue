  ...
  <div class="info-form">
    <el-form ref="infoForm" :model="infoForm" size="small" label-width="100px">
      <el-form-item label="岗位名称" prop="name" :rules="[{
                    required: true, message: '岗位名称名称不能为空', trigger: 'blur'
                  },{min: 2, max: 20, message: '岗位名称长度在2 到 20 个字符', trigger: 'blur'}]">
        <el-input maxlength="20" clearable v-model="infoForm.name" size="small" placeholder="请输入岗位名称"/>
      </el-form-item>
    ...
    methods: {
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            let method = 'positionAdd'
            if (this.infoForm.id) {
              method = 'positionEdit'
            }
            this.addBtnLoading = true
            positionApi[method](this.infoForm).then(res => {
